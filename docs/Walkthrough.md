...
`Hooks` form the core functionality for Akd. A Hook is an abstraction around a
set of operations which are to be performed in an ordered way. Operations
include the commands to be ran on a pseudo-terminal and the destination where
the commands are to be run. These operations can be categoried in three sets:
`main`, `ensure` and `rollback`. Main includes the operations which denote the
actual functionality of the hook. It could be running a migration, fetching
source code etc.Ensure includes the operations which are to be ran after the
main set of operations succeed. These usually include cleaning up, deleting temp
files etc.  Rollback includes the operations which are to be ran if a deployment
fails. They are meant to negate the effects of the main operations and include
operations like rolling back a migration, putting back old source code etc.

A `Deployment` is a collection of hooks with a `mix_env` and information about
where to build and publish an app. A Deployment (by default) has multiple
phases (pipeline of hooks):
...
These phases are generated by akd, by default, but they are all optional. It
is also very easy to create your own phase using akd's pipeline DSL, which we
will cover in other Walkthroughs.

Akd works by executing a deployment's hooks in order, which means executing
the main operations of the hooks and transforming the deployment struct as
the deployment proceeds in the hooks pipeline. Akd provides DSLs to define
that pipeline and hooks, which make customizing a deployment task a breeze
without having to know a lot about akd's underlying workflow. Even akd's
base hooks use Akd's DSL, therefore they are easier to read/modify/duplicate.
